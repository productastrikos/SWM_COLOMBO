...
import KPICard, { IcoLeaf, IcoRecycle, IcoBarChart, IcoRoute } from '../components/KPICard';
...
export default function Sustainability() {
  ...
  const [emissionsFrame, setEmissionsFrame] = useState('30D');
  const activeEmissionsFrame = getTimeframeOption(TIMEFRAME_OPTIONS.monthly, emissionsFrame);

  const CO2_12M = [180, 172, 165, 158, 152, 148, 145, 142, 140, 138, 136, k.co2Tons || 135];
    ...
    datasets: [
      {
        ...
        data: resampleSeries(
          activeEmissionsFrame.dataWindow ? CO2_12M.slice(-activeEmissionsFrame.dataWindow) : CO2_12M,
          activeEmissionsFrame.points
        ),
  ...
  const greenScoreData = {
    ...
    datasets: [{
      data: greenScoreVals,
      backgroundColor: greenScoreVals.map(v => v >= 70 ? t.successBar : v >= 55 ? t.warningBar : t.dangerBar),
      borderColor: greenScoreVals.map(v => v >= 70 ? t.success : v >= 55 ? t.warning : t.danger),
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        <KPICard icon={<IcoLeaf />} label="Carbon Score" value={`${(k.carbonScore || 72).toFixed(0)}/100`}
          ...
          onClick={() => setSelectedKPI({
            icon: <IcoLeaf />, label: 'Carbon Score', value: (k.carbonScore || 72).toFixed(0), unit: '/100',
        ...
        <KPICard icon={<IcoRecycle />} label="Waste Diversion" value={`${(k.diversionRate || 58).toFixed(0)}%`}
          ...
          trend={1.4}
          onClick={() => setSelectedKPI({
            icon: <IcoRecycle />, label: 'Waste Diversion', value: (k.diversionRate || 58).toFixed(0), unit: '%',
        ...
        <KPICard icon={<IcoBarChart />} label="CO₂ Reduction" value={`${(k.co2Reduction || 12.5).toFixed(1)}%`}
          ...
          trend={0.8}
          onClick={() => setSelectedKPI({
            icon: <IcoBarChart />, label: 'CO₂ Reduction', value: (k.co2Reduction || 12.5).toFixed(1), unit: '%',
        ...
        <KPICard icon={<IcoRoute />} label="Circular Economy" value={`${(k.circularIndex || 47).toFixed(0)}%`}
          ...
          trend={3.2}
          onClick={() => setSelectedKPI({
            icon: <IcoRoute />, label: 'Circular Economy', value: (k.circularIndex || 47).toFixed(0), unit: '%',
      ...
      <div className="bg-cwm-panel border border-cwm-border rounded-lg p-3">
        ...
        <div style={{ height: 140 }}>
          <Bar data={greenScoreData} options={{
            ...
            indexAxis: 'y',
            scales: chartScales({ x: { max: 100 }, y: { grid: { display: false } } }),
          }} />
        </div>
      </div>
